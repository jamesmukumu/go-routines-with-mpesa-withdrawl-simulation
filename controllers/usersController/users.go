...
func(customer *Withdrawamount)ValidatePin()bool{
...
var Transactamount Withdrawamount
	...
	//decode pin and amount

	decodedTransactamountandpin := json.NewDecoder(req.Body).Decode(&Transactamount)
	if decodedTransactamountandpin != nil {
		log.Panic(decodedTransactamountandpin.Error())
	} 

	if Mycustomer.Balance < Transactamount.Amount {
		fmt.Println("Insufficient funds")
		res.Write([]byte("insufficient funds"))
		return
	}
	...
	if !Transactamount.ValidatePin(){
		res.Write([]byte("please Provide pin and it should be 4 characters long"))
		return
	}

	// validate pin
	matchingPin := bcrypt.CompareHashAndPassword([]byte(Mycustomer.Pin), []byte(Transactamount.Pin))
	if matchingPin != nil {
